...
function Feed(props) {
    return (
        <div id='feed-wrapper'>
            <Title />
            {
                [1, 2, 3, 4, 5, 6, 7, 8, 9].map(x => <SinglePost key={x} />)
            }
        </div>
    )
}
...
function Content(props) {
    return (
        <div className='post-cont'>
            <div>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ipsam nesciunt velit qui dolorem quae quibusdam eum, aliquid reprehenderit deserunt animi culpa accusantium illum voluptates veniam. Saepe, consectetur. Similique, quibusdam possimus.</div>
        </div>
    )
}
...
function Media(props) {
    return (
        <img src={Forest} alt="media" className='post-media' />
    )
}
